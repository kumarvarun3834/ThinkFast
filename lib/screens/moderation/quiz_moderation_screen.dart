...
class _QuizModerationScreenState extends State<QuizModerationScreen>
  ...
  void initState() {
    ...
    _participantsStream = global.qDb.getQuizParticipants(widget.quizId);
  }
  ...
  Future<void> _handleBulkAction() async {
    ...
    final bool isBannedTab = tabIndex == 0;
    final bool isDeletedTab = tabIndex == 1;
    final bool isParticipantsTab = tabIndex == 2;

    if (!(_isAdmin || (global.featureFlags?['management_features'] ?? true))) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Access Denied: Moderation features are currently disabled platform-wide.",
          ),
    ...
    // Permission check
    String requiredPerm = 'canModerate';
    if (isBannedTab) requiredPerm = 'can_ban_users';
    if (isParticipantsTab) requiredPerm = 'can_manage_collaborators';
    ...
    if (isBannedTab) {
      ...
      content = "These users will be allowed to take the quiz again.";
      actionLabel = "UNBLOCK";
    } else if (isDeletedTab) {
      title = "Recover Selected?";
      content = "These responses will be restored to active status.";
      actionLabel = "RECOVER";
    } else {
      title = "Revoke Access?";
      content =
          "These users will no longer have participant access to this quiz.";
      actionLabel = "REVOKE";
    }
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: isParticipantsTab
                  ? global.errorColor
                  : global.successColor,
            ),
            ...
            child: Text(
              actionLabel,
              style: TextStyle(
                color: isParticipantsTab ? Colors.white : Colors.black,
              ),
            ),
          ),
        ],
    ...
    if (confirm == true) {
      ...
      try {
        if (isBannedTab) {
          ...
          for (String id in _selectedIds) {
            await global.qDb.restoreResponse(
              responseId: id,
              quizId: widget.quizId,
            );
          }
      ...
      appBar: AppBar(
        ...
        actions: _isSelectionMode
            ? [
                TextButton.icon(
                  ...
                  label: Text(
                    _tabController.index == 0 ? "UNBLOCK" : "RECOVER",
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(text: "ALLOWED USERS", icon: Icon(Icons.lock_open_rounded)),
          ],
        ),
      ),
      body: TabBarView(
  ...
  void _confirmRevokeAccess(Map<String, dynamic> user) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: _cardColor,
        title: Text(
          "Revoke Access for ${user['userName'] ?? 'User'}?",
          style: const TextStyle(color: Colors.white),
        ),
        actions: [
          TextButton(
