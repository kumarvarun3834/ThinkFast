...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
    ...
    try {
      ...
      if (_activeCategory == "Overview" || _activeCategory == "System Health") {
        final responses = await Future.wait([
          http.get(Uri.parse("${global.aiBackendUrl}/api/health"), headers: headers),
          http.get(Uri.parse("${global.aiBackendUrl}/api/admin/metrics"), headers: headers),
        ]);
      ...
      if (_activeCategory == "Overview" || _activeCategory == "Active Monitoring") {
        final responses = await Future.wait([
          http.get(Uri.parse("${global.aiBackendUrl}/api/active-quizzes"), headers: headers),
          http.get(Uri.parse("${global.aiBackendUrl}/api/notifications"), headers: headers),
        ]);
  ...
  Future<void> _triggerAction(String path, {Map<String, dynamic>? body, bool destructive = false}) async {
    if (destructive) {
      final confirm = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text("Are you sure you want to trigger $path? This action may be irreversible.", style: const TextStyle(color: Colors.white70)),
          actions: [
    ...
    try {
      ...
      if (mounted) {
        messenger.showSnackBar(
          SnackBar(
            content: Text(response.statusCode == 200 ? "Success" : "Error: ${response.statusCode}"),
  ...
  Widget build(BuildContext context) {
    final bool isEnabled = global.featureFlags?['enable_admin_dashboard'] ?? true;
      ...
      return Scaffold(
        ...
        body: Center(
          child: Column(
            ...
            children: [
              const Icon(Icons.lock_person_rounded, color: global.errorColor, size: 64),
              const SizedBox(height: 24),
              Text(
                "Dashboard Disabled",
                style: GoogleFonts.poppins(color: global.valueColor, fontSize: 20, fontWeight: FontWeight.bold),
              ),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: const Icon(Icons.refresh_rounded, color: global.primaryAccent, size: 20),
  ...
  Widget _buildSidebar(bool isSmall) {
    ...
    if (isSmall) {
      return NavigationRail(
        backgroundColor: Colors.transparent,
        selectedIndex: categories.indexWhere((e) => e['name'] == _activeCategory),
        ...
        destinations: categories.map((cat) {
          return NavigationRailDestination(
            ...
            selectedIcon: Icon(cat['icon'] as IconData, color: global.primaryAccent),
    ...
    return Container(
      width: 240,
      decoration: const BoxDecoration(border: Border(right: BorderSide(color: global.borderColor))),
      child: ListView(
        children: categories.map((cat) {
          final String name = cat['name'] as String;
          final bool isActive = _activeCategory == name;
          return ListTile(
            leading: Icon(cat['icon'] as IconData, color: isActive ? global.primaryAccent : global.labelColor, size: 20),
            title: Text(
              name,
            ...
            selected: isActive,
            selectedTileColor: global.primaryAccent.withValues(alpha: 0.1),
          );
        }).toList(),
      ),
    );
  ...
  Widget _buildOverview(bool isSmall) {
    ...
    return ListView(
      ...
      children: [
        Text("System Summary", style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.bold, color: global.valueColor)),
        ...
        GridView.count(
          ...
          children: [
            _buildInfoTile("Status", health['status']?.toString() ?? "...", Icons.dns_rounded, 
                color: health['status'] == "UP" ? Colors.greenAccent : Colors.redAccent),
            _buildInfoTile("Uptime", metrics['uptime']?.toString() ?? "0s", Icons.timer_rounded),
            _buildInfoTile("Memory", metrics['memory']?.toString() ?? "0 MB", Icons.memory_rounded),
            _buildInfoTile("Sessions", quizzes.length.toString(), Icons.bolt_rounded, color: global.primaryAccent),
            _buildInfoTile("Notifs", _getList('notifications').length.toString(), Icons.notifications_active_rounded),
        ...
        Wrap(
          ...
          children: [
            _buildActionButton(
              onPressed: () => _triggerAction("/api/admin/config", body: {"latency": 0}),
              icon: Icons.speed_rounded,
              label: "Zero Latency",
            ),
            _buildActionButton(
              onPressed: () => _triggerAction("/api/database/simulate-client-write"),
  ...
  Widget _buildMonitoring() {
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        if (quizzes.isEmpty)
          const Text("No active sessions currently.", style: TextStyle(color: global.labelColor))
        else
          ...quizzes.map((q) => _buildListItem(q.toString(), Icons.quiz_rounded)),
        const SizedBox(height: 32),
        _buildSectionHeader("Notification Queue (${notifications.length})"),
        const SizedBox(height: 16),
        if (notifications.isEmpty)
          const Text("Notification queue is empty.", style: TextStyle(color: global.labelColor))
        else
          ...notifications.map((n) => _buildListItem(n.toString(), Icons.notification_important_rounded)),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  label,
                  style: GoogleFonts.poppins(color: global.labelColor, fontSize: 9, fontWeight: FontWeight.bold),
                  overflow: TextOverflow.ellipsis,
                ),
              ),
            ],
          ...
          Text(
            value,
            style: GoogleFonts.poppins(color: value == "Error" ? Colors.redAccent : global.valueColor, fontSize: 14, fontWeight: FontWeight.bold),
            overflow: TextOverflow.ellipsis,
            maxLines: 1,
          ),
        ],
  ...
  Widget _buildSectionHeader(String title) {
    return Row(
      children: [
        Container(width: 4, height: 16, decoration: BoxDecoration(color: global.primaryAccent, borderRadius: BorderRadius.circular(2))),
        const SizedBox(width: 12),
        Text(title.toUpperCase(), style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.bold, color: global.primaryAccent, letterSpacing: 1.2)),
      ],
    );
  }
  ...
  Widget _buildActionCard(String title, String desc, IconData icon, VoidCallback onTap, {bool isDestructive = false}) {
    return Container(
      decoration: BoxDecoration(
        color: global.cardColor,
        borderRadius: BorderRadius.circular(12),
      ...
      child: ListTile(
        ...
        leading: Icon(icon, color: isDestructive ? Colors.redAccent : global.primaryAccent, size: 20),
        title: Text(
          title,
          style: const TextStyle(color: global.valueColor, fontWeight: FontWeight.bold, fontSize: 13),
        ),
        subtitle: Text(
          desc,
        ...
        trailing: ElevatedButton(
          style: ElevatedButton.styleFrom(
            backgroundColor: isDestructive ? Colors.redAccent : global.btnColor,
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              text,
              style: GoogleFonts.firaCode(color: global.valueColor, fontSize: 10),
              overflow: TextOverflow.ellipsis,
              maxLines: 1,
            ),
          ),
